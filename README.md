...
> Panel de administración de e-commerce. Gestiona 5.000 órdenes con filtros, ordenación y paginación server-side sin degradar el rendimiento.
...
El resto de este documento (arquitectura, MSW, pilares técnicos) describe el **diseño objetivo** del proyecto, no todo está construido todavía. Este es el corte real, para no hablar en futuro de cosas que no existen:
...
## 🏛️ Arquitectura y estructura (objetivo)
...
FSD organiza el código en capas con una regla de dependencia estricta: una capa solo puede importar de las capas inferiores.

```
app  →  pages  →  widgets  →  features  →  entities  →  shared
```

`entities/order` (el modelo de negocio "orden") nunca depende de `features/filter-orders`; `shared` no sabe nada del dominio del negocio. Cada slice expone su API pública a través de un único `index.ts`.

### Árbol de carpetas objetivo

> ⚠️ Este árbol representa el diseño al que apunta el proyecto (ver [Estado actual](#-estado-actual)), no la estructura ya implementada.
...
Las variables se consumen de forma centralizada y tipada en [`src/shared/config/env.ts`](src/shared/config/env.ts), que valida en runtime que las variables requeridas existan antes de arrancar la app:
